...
def parse_args():
    ...
    parser.add_argument('--get_embs', default=False, action='store_true')
    return parser.parse_args()

def main(config, get_embs):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="emdcfl")
    ...
    embs_folder = None
    if get_embs:
        embs_folder = f"embs/embs_{config['dataset']}m{config['model_type']}LR{config['lr']}"
        if not os.path.exists(embs_folder):
            os.makedirs(embs_folder)

    start_time = time.time()
    trainer.fit_emdcfl(server, centers, center_trains, center_vals, config["tolerance"], config["num_clusters"], embs_folder, config["proj_ratio"])
    end_time = time.time()
    elapsed_time = end_time - start_time
    with open(f'wd_{config["dataset"]}.txt', "w") as file:
...
if __name__ == "__main__":
    ...
    if args.batch_config is None:
        print(config)
        log_dir = main(config, args.get_embs)
