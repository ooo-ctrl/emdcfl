...
def main(config):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="fedavg")
    ...
    trainer.fit_fedavg(server, centers, center_trains, center_vals)
    end_time = time.time()
    elapsed_time = end_time - start_time
    with open(f'fedavg_{config["dataset"]}.txt', "w") as file:
