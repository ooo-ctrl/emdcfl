...
from src.center import Center, CenterFedProx
...
def main(config):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="fedprox")

    mu = 0.01 # config["mu"]
    centers = [CenterFedProx(id, copy.deepcopy(model), optimizer=config['optimizer'], lr=config['lr'], device=device, mu=mu) for id in range(config["num_centers"]) ]
    ...
    trainer.fit_fedprox(server, centers, center_trains, center_vals)
