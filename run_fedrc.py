...
from src.center import Center, CenterRC
...
def main(config):
    ...
    models = [load_model(model_type=config['model_type'], label_len=len(load_labels(config["dataset"]))) for _ in range(config['num_clusters'])]
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="fedrc")

    centers = [CenterRC(id, copy.deepcopy(models[0]), optimizer=config['optimizer'], lr=config['lr'], device=device, num_clusters=config["num_clusters"], num_classes=len(load_labels(config["dataset"])), n_train=len(load_data(config["dataset"], "train", id))) for id in range(config["num_centers"]) ]
    servers = [Center(-1, copy.deepcopy(models[cluster_id]), optimizer=config['optimizer'], lr=config['lr'], device=device, cluster_id=cluster_id) for cluster_id in range(config["num_clusters"]) ]
    ...
    trainer.fit_fedrc(servers, centers, center_trains, center_vals)
