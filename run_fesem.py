...
from src.center import CenterFedProx, Center
...
def main(config):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="fesem")

    mu = 0.1 # config["mu"]
    centers = [CenterFedProx(id, copy.deepcopy(models[0]), optimizer=config['optimizer'], lr=config['lr'], device=device, mu=mu) for id in range(config["num_centers"]) ]
    ...
    trainer.fit_fesem(servers, centers, center_trains, center_vals)
