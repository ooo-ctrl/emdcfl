...
def main(config):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="flexcfl")
    ...
    trainer.fit_flexcfl(centers, center_trains, center_vals, config["num_clusters"])
