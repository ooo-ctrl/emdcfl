...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='config_p.yaml', help='Path to config file')
...
def main(config):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="gt")
    ...
    if "rot" in config["dataset"]:
        if "max" in config["dataset"]:
            num_clusters = 4
        elif "min" in config["dataset"]:
            num_clusters = 1
        elif "no" in config["dataset"]:
            num_clusters = 1
        else:
            num_clusters = 2

    trainer.fit_gt(server, centers, center_trains, center_vals, config["dataset"], num_clusters, config["p_num"])
