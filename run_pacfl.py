...
def main(config):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="pacfl")
    ...
    tau = 0
    if "rotcifar" in config["dataset"]:
        tau = 12
    elif "rotmnist" in config["dataset"]:
        tau = 12
    elif "pacs500" in config["dataset"]:
        tau = 11
    elif "office" in config["dataset"]:
        tau = 14
    elif "cmnist" in config["dataset"]:
        tau = 2.5
    elif "cifar10-ls" in config["dataset"]:
        tau = 10.5
    assert tau > 0
    ...
    trainer.fit_pacfl(server, centers, center_trains, center_vals, config["dataset"])
