...
from src.center import CenterPFedGraph
...
def main(config):
    ...
    writer = get_summarywriter(out_dir=config['out_dir'], num_centers=config["num_centers"], label_len=len(load_labels(config["dataset"])), method="pfedgraph")

    mu = 0.01 # config["mu"]
    centers = [CenterPFedGraph(id, copy.deepcopy(model), optimizer=config['optimizer'], lr=config['lr'], device=device, mu=mu) for id in range(config["num_centers"]) ]
    ...
    trainer.fit_pfedgraph(centers, center_trains, center_vals)
